...
class AddPostForm(forms.ModelForm):
	flag = False # для удобной тестировки
	captcha = CaptchaField(label='Enter a code', required=flag)
	...
	class Meta:
		...
		widgets = {
			'title': forms.TextInput(attrs={'class': 'input', 'placeholder': ' Name Surname'}), # класс из нашего цсс для установления стиля поля
	...
	для создания пользовательских валидаторов, в форме нужно прописать метод
	...
	# 	if len(title) > 25:
	# 		raise ValidationError('Длина заголовка превышает 25 символов')
	# 	return title

	# def clean_content(self):
	# 	content = self.cleaned_data['content']
	# 	if len(content) < 50:
	# 		raise ValidationError('Длина статьи должна быть не менее 50-ти символов')
	# 	return content

	# def clean_slug(self):
	# 	slug = self.cleaned_data['slug']
	# 	if slug != slug.lower():
	# 		raise ValidationError('URL должен быть в нижнем регистре')
	# 	return slug

class RegisterUserForm(UserCreationForm): #расширяем джанговский класс UserCreationForm(он является стандартным от джанго)
	username = forms.CharField(label="Login", widget=forms.TextInput(attrs={'class': 'form-input', 'placeholder': ' login'}))
	email = forms.EmailField(label='Email', required=False, widget=forms.EmailInput(attrs={'class': 'form-input', 'placeholder': ' example@email.com'}))
...
class ContactForm(forms.Form):
	name = forms.CharField(label='Name', max_length=255, widget=forms.TextInput(attrs={'class': 'form-input', 'placeholder': 'Your name'}))
	email = forms.EmailField(label='Email', required=False, widget=forms.EmailInput(attrs={'class': 'form-input', 'placeholder': 'example@gmail.com'}))
