...
class WomenAdmin(admin.ModelAdmin):
	...
	""" list_display для отображения доп. полей в админке """
...
# Без применения декоратора, можно зарегать модели след. образом:
# admin.site.register(Women, WomenAdmin)
# admin.site.register(Category, CategoryAdmin)


""" Таким способом можно изменять атрибуты для админ.панели(полный список смотреть в документации). Эти 
параметры используются в html шаблонах, в частности в кастомном шаблоне base_site.html 
(который лежит в 'templates/admin'), который мы переопеределили от стандартного """
